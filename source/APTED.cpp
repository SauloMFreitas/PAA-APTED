...
template<typename T>
static inline void fillArray(vector<T> &array, T val) {
...
template<class Data>
void Apted<Data>::updateFnArray(int lnForNode, int node, int currentSubtreePreL) {
...
template<class Data>
void Apted<Data>::updateFtArray(int lnForNode, int node) {
    ft[node] = lnForNode;
    if(fn[node] > -1) {
        ft[fn[node]] = node;
    }
...
template<class Data>
int Apted<Data>::getStrategyPathType(int pathIDWithPathIDOffset, int pathIDOffset, NodeIndexer<Data>* it, int currentRootNodePreL, int currentSubtreeSize) {
    if (signum(pathIDWithPathIDOffset) == -1) {
        return LEFT;
    }
    int pathID = abs(pathIDWithPathIDOffset) - 1;
...
template<class Data>
float Apted<Data>::spfA(NodeIndexer<Data>* it1, NodeIndexer<Data>* it2, int pathID, int pathType, bool treesSwapped) {
    vector<Node<Data>*> &it2nodes = it2->preL_to_node;
    Node<Data>* lFNode;
    vector<int> &it1sizes = it1->sizes;
    vector<int> &it2sizes = it2->sizes;
    vector<int> &it1parents = it1->parents;
    vector<int> &it2parents = it2->parents;
    vector<int> &it1preL_to_preR = it1->preL_to_preR;
    vector<int> &it2preL_to_preR = it2->preL_to_preR;
    vector<int> &it1preR_to_preL = it1->preR_to_preL;
    vector<int> &it2preR_to_preL = it2->preR_to_preL;
    ...
    vector<vector<float>> t(subtreeSize2 + 1);
    for (size_t i = 0; i < t.size(); i++) {
        t[i].resize(subtreeSize2 + 1);
    }
    vector<vector<float>> s(subtreeSize1 + 1);
    ...
    vector<float>* sp1spointer;
    vector<float>* sp2spointer;
    vector<float>* sp3spointer;
    vector<float>* sp3deltapointer;
    vector<float>* swritepointer;
    vector<float>* sp1tpointer;
    vector<float>* sp3tpointer;
...
template<class Data>
float Apted<Data>::spfL(NodeIndexer<Data>* it1, NodeIndexer<Data>* it2, bool treesSwapped) {
    // Initialise the array to store the keyroot nodes in the right-hand input subtree.
    vector<int> keyRoots(it2->sizes[it2->getCurrentNode()], -1);
    ...
    vector<vector<float>> forestdist(it1->sizes[it1->getCurrentNode()] + 1);
...
template<class Data>
int Apted<Data>::computeKeyRoots(NodeIndexer<Data>* it2, int subtreeRootNode, int pathID, vector<int> &keyRoots, int index) {
...
template<class Data>
void Apted<Data>::treeEditDist(NodeIndexer<Data>* it1, NodeIndexer<Data>* it2, int it1subtree, int it2subtree, vector<vector<float>> &forestdist, bool treesSwapped) {
...
template<class Data>
float Apted<Data>::spfR(NodeIndexer<Data>* it1, NodeIndexer<Data>* it2, bool treesSwapped) {
    // Initialise the array to store the keyroot nodes in the right-hand input subtree.
    vector<int> revKeyRoots(it2->sizes[it2->getCurrentNode()], -1);
...
template<class Data>
int Apted<Data>::computeRevKeyRoots(NodeIndexer<Data>* it2, int subtreeRootNode, int pathID, vector<int> &revKeyRoots, int index) {
    // The subtreeRootNode is a keyroot node. Add it to keyRoots.
...
template<class Data>
void Apted<Data>::revTreeEditDist(NodeIndexer<Data>* it1, NodeIndexer<Data>* it2, int it1subtree, int it2subtree, vector<vector<float>> &forestdist, bool treesSwapped) {
...
template<class Data>
float Apted<Data>::spf1 (NodeIndexer<Data>* ni1, int subtreeRootNode1, NodeIndexer<Data>* ni2, int subtreeRootNode2) {
...
template<class Data>
void Apted<Data>::computeOptStrategy_postL() {
    int size1 = this->ind1->getSize();
    int size2 = this->ind2->getSize();
    ...
    vector<vector<float>> cost1_L(size1);
    vector<vector<float>> cost1_R(size1);
    vector<vector<float>> cost1_I(size1);
    vector<float> cost2_L(size2);
    vector<float> cost2_R(size2);
    vector<float> cost2_I(size2);
    vector<int> cost2_path(size2);
    vector<float> leafRow(size2);
    int pathIDOffset = size1;
    float minCost = 0x7fffffffffffffffL;
    int strategyPath = -1;

    vector<int> &pre2size1 = this->ind1->sizes;
    vector<int> &pre2size2 = this->ind2->sizes;
    vector<int> &pre2descSum1 = this->ind1->preL_to_desc_sum;
    vector<int> &pre2descSum2 = this->ind2->preL_to_desc_sum;
    vector<int> &pre2krSum1 = this->ind1->preL_to_kr_sum;
    vector<int> &pre2krSum2 = this->ind2->preL_to_kr_sum;
    vector<int> &pre2revkrSum1 = this->ind1->preL_to_rev_kr_sum;
    vector<int> &pre2revkrSum2 = this->ind2->preL_to_rev_kr_sum;
    vector<int> &preL_to_preR_1 = this->ind1->preL_to_preR;
    vector<int> &preL_to_preR_2 = this->ind2->preL_to_preR;
    vector<int> &preR_to_preL_1 = this->ind1->preR_to_preL;
    vector<int> &preR_to_preL_2 = this->ind2->preR_to_preL;
    vector<int> &pre2parent1 = this->ind1->parents;
    vector<int> &pre2parent2 = this->ind2->parents;
    vector<bool> &nodeType_L_1 = this->ind1->nodeType_L;
    vector<bool> &nodeType_L_2 = this->ind2->nodeType_L;
    vector<bool> &nodeType_R_1 = this->ind1->nodeType_R;
    vector<bool> &nodeType_R_2 = this->ind2->nodeType_R;

    vector<int> &preL_to_postL_1 = this->ind1->preL_to_postL;
    vector<int> &preL_to_postL_2 = this->ind2->preL_to_postL;
    vector<int> &postL_to_preL_1 = this->ind1->postL_to_preL;
    vector<int> &postL_to_preL_2 = this->ind2->postL_to_preL;
    ...
    vector<float> cost_Lpointer_v,
                    cost_Rpointer_v,
                    cost_Ipointer_v;
    vector<float> cost_Lpointer_parent_v, 
                    cost_Rpointer_parent_v,
                    cost_Ipointer_parent_v;
    vector<float> strategypointer_parent_v;
    ...
    stack<vector<float>> rowsToReuse_L;
    stack<vector<float>> rowsToReuse_R;
    stack<vector<float>> rowsToReuse_I;

    for(int v = 0; v < size1; v++) {
        v_in_preL = postL_to_preL_1[v];

        is_v_leaf = this->ind1->isLeaf(v_in_preL);
        ...
        if (parent_v_preL != -1 && cost1_L[parent_v_postL].size() == 0) {
            if (rowsToReuse_L.empty()) {
                cost1_L[parent_v_postL] = vector<float>(size2);
                cost1_R[parent_v_postL] = vector<float>(size2);
                cost1_I[parent_v_postL] = vector<float>(size2);
            } else {
...
template<class Data>
void Apted<Data>::computeOptStrategy_postR() {
    int size1 = this->ind1->getSize();
    int size2 = this->ind2->getSize();
    ...
    for(int v = size1 - 1; v >= 0; v--) {
        ...
        if (parent_v != -1 && cost1_L[parent_v].size() == 0) {
            if (rowsToReuse_L.empty()) {
                cost1_L[parent_v] = vector<float>(size2);
                cost1_R[parent_v] = vector<float>(size2);
                cost1_I[parent_v] = vector<float>(size2);
            } else {
...
template<class Data>
void Apted<Data>::tedInit() {
    // Reset the subproblems counter.
    counter = 0L;

    // Initialize arrays.
    int maxSize = Max(this->size1, this->size2) + 1;
    ...
    for(int x = 0; x < this->size1; x++) {
        sizeX = this->ind1->sizes[x];
        // parentX = this->ind1->parents[x];

        for(int y = 0; y < this->size2; y++) {
            sizeY = this->ind2->sizes[y];
            // parentY = this->ind2->parents[y];
            ...
            if (sizeX == 1 && sizeY == 1) {
                ...
                delta[x][y] = this->ind2->preL_to_sumInsCost[y] - this->costModel->insertCost(this->ind2->preL_to_node[y]); // USE COST MODEL.
            } else if (sizeY == 1) {
                delta[x][y] = this->ind1->preL_to_sumDelCost[x] - this->costModel->deleteCost(this->ind1->preL_to_node[x]); // USE COST MODEL.
            }
        }
    }
}
...
template<class Data>
float Apted<Data>::gted(NodeIndexer<Data>* it1, NodeIndexer<Data>* it2) {
    ...
    int currentPathNode = abs(strategyPathID) - 1;
...
template<class Data>
Apted<Data>::Apted(CostModel<Data>* costModel) : TreeEditDistace<Data>(costModel) {}

template<class Data>
float Apted<Data>::computeEditDistance(Node<Data>* t1, Node<Data>* t2) {
        ...
        if (this->ind1->lchl < this->ind1->rchl) {
        ...
        return gted(this->ind1, this->ind2);
    }